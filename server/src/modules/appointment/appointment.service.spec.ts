...
describe('AppointmentService', () => {
  ...
  describe('findAllAppointmentsBookedByCustomer', () => {
    it('should return dashboard statistics for full year', async () => {
      // Đảm bảo mock trả về mảng rỗng nếu không có dữ liệu
      const mockAppointments = [
        {
          ...
          customer: { id: 'customer-1', full_name: 'Test Customer' },
          details: [
            { quantity: 2, service: { id: 'service-1', name: 'Test Service' } },
          ],
          createdAt: new Date(2024, 5, 1),
        },
      ];
      // Mock both appointmentRepository.find for getDashboard
    ...
    it('should create appointment with valid service', async () => {
  ...
  describe('getDashboard', () => {
    it('should return dashboard statistics for full year', async () => {
      ...
      // Mock appointments for getDashboard
      const mockAppointments = [
        {
          id: 'appt-1',
          totalAmount: 200000,
  ...
  describe('requestCancelByDoctorBulk', () => {
    it('should create cancel requests for valid appointments', async () => {
      ...
      // Thêm mock cho internalRepo.find
      internalRepository.find = jest.fn().mockResolvedValue([{ id: 'staff-1', full_name: 'Test Staff', email: 'staff@test.com' }]);
      // Thêm mock cho appointmentRepository.update
  ...
  describe('rejectRequest', () => {
    it('should reject cancel request successfully', async () => {
      ...
      (service as any).cancelRepo = mockCancelRequestRepository;
      (service as any).notificationService = mockNotificationService;
      // Thêm mock cho appointmentRepo.update
      appointmentRepository.update = jest.fn().mockResolvedValue({});
      // Mock findOne để trả về appointment với đầy đủ relations
      appointmentRepository.findOne.mockResolvedValue({
        ...mockAppointment,
        id: 'appointment-1',
        customer: mockCustomer,
