...
describe('AccountService', () => {
    ...
    describe('checkDuplicateEmailWithRole', () => {
        ...
        it('returns Admin when internal with admin role exists', async () => {
            customerRepository.findOne.mockResolvedValue(null);
            doctorRepository.findOne.mockResolvedValue(null);
            internalRepository.findOne.mockResolvedValue({ role: { name: 'admin' } });

            const result = await service.checkDuplicateEmailWithRole('admin@x.com');
            expect(result).toBe(RoleEnum.Admin);
        });

        it('returns Cashier when internal with cashier role exists', async () => {
            customerRepository.findOne.mockResolvedValue(null);
            doctorRepository.findOne.mockResolvedValue(null);
            internalRepository.findOne.mockResolvedValue({ role: { name: 'cashier' } });

            const result = await service.checkDuplicateEmailWithRole('cashier@x.com');
            expect(result).toBe(RoleEnum.Cashier);
        });

        it('returns null when internal has unknown role', async () => {
            customerRepository.findOne.mockResolvedValue(null);
            doctorRepository.findOne.mockResolvedValue(null);
            internalRepository.findOne.mockResolvedValue({ role: { name: 'unknown' } });

            const result = await service.checkDuplicateEmailWithRole('unknown@x.com');
            expect(result).toBeNull();
        });
    });
    ...
    describe('Customer Management', () => {
        describe('createCustomer', () => {
            ...
            it('should throw error when email already exists', async () => {
                ...
                await expect(service.createCustomer(createCustomerDto)).rejects.toThrow('Email đã được sử dụng');
            });
            ...
            it('should throw error when email is missing', async () => {
                ...
                await expect(service.createCustomer(createCustomerDto)).rejects.toThrow('Email cần được cung cấp');
            });
        });
    ...
    describe('Internal Management', () => {
        describe('createInternal', () => {
            ...
            it('should throw error when role is invalid', async () => {
                ...
                await expect(service.createInternal(createInternalDto)).rejects.toThrow('Chức vụ không hợp lệ');
            });

            it('should throw error when trying to create admin role', async () => {
                const createInternalDto: any = {
                    full_name: 'New Admin',
                ...
                const adminRole = { id: 1, name: 'admin', description: 'Administrator' };
    ...
    describe('Doctor Management', () => {
        describe('createDoctor', () => {
            ...
            it('should create doctor with services when serviceIds provided', async () => {
                const mockServices = [
                ...
                internalRepository.findOne.mockResolvedValue(null);
                doctorRepository.findOne.mockResolvedValue(null);
                serviceRepository.findByIds.mockResolvedValue(mockServices);
                doctorRepository.create.mockReturnValue({
                    ...createDoctorDto,
                ...
                expect(serviceRepository.findByIds).toHaveBeenCalledWith(['service-1', 'service-2']);
                expect(result).toHaveProperty('id', 'new-doctor-id');
            });
        });
        ...
        describe('updateDoctor', () => {
            ...
            it('should update doctor with new services when serviceIds provided', async () => {
                const mockServices = [
                    { id: 'service-1', name: 'Service 1' },
                    { id: 'service-2', name: 'Service 2' },
                ];
                ...
                const doctorWithOldServices = {
                    ...mockDoctor,
                    services: [{ id: 'old-service', name: 'Old Service' }],
                };

                doctorRepository.findOne.mockResolvedValue(doctorWithOldServices);
                serviceRepository.find.mockResolvedValue(mockServices);
                doctorRepository.save.mockResolvedValue({
                    ...doctorWithOldServices,
                    ...updateDto,
                    services: mockServices,
                });
